-- ============================================================
-- ROW LEVEL SECURITY — v3 WITH BYPASS RLS SUPPORT
-- ============================================================
-- Mọi policy đều cho phép bypass khi:
--   current_setting('app.bypass_rls', TRUE) = 'true'
-- Dùng cho: IPN callbacks, background jobs, admin operations
-- ============================================================
...
-- Helper function để check bypass
...
USING (
    ...
    OR (current_farm_id() IS NOT NULL AND farm_id = current_farm_id())
);
...
-- DELETE vẫn bị block hoàn toàn kể cả bypass (audit trail)
-- ============================================================
...
-- DELETE luôn bị block — audit trail bất khả xâm phạm
...
-- ============================================================
-- 7. AUDIT_LOGS
-- INSERT/UPDATE/DELETE vẫn bị block kể cả bypass
-- ============================================================
...
-- INSERT/UPDATE/DELETE luôn bị block — chỉ qua SECURITY DEFINER function
...
-- ============================================================
-- 10. CÁC BẢNG CÓ farm_id TRỰC TIẾP — STRICT WITH BYPASS
-- ============================================================
...
USING (
    is_bypass_rls()
    OR plan_id IN (SELECT id FROM plans WHERE farm_id = current_farm_id())
);
...
USING (
    is_bypass_rls()
    OR task_id IN (SELECT id FROM tasks WHERE farm_id = current_farm_id())
);
...
-- Immutable audit trail — UPDATE/DELETE luôn bị block
...
USING (
    is_bypass_rls()
    OR work_log_id IN (SELECT id FROM work_logs WHERE farm_id = current_farm_id())
);
...
USING (
    is_bypass_rls()
    OR diagnosis_id IN (SELECT id FROM diagnoses WHERE farm_id = current_farm_id())
);
...
WITH CHECK (
    is_bypass_rls()
    OR crop_id IN (SELECT id FROM crops WHERE scope = 'FARM' AND farm_id = current_farm_id())
);
...
USING (
    is_bypass_rls()
    OR crop_id IN (SELECT id FROM crops WHERE farm_id = current_farm_id() AND scope = 'FARM')
);
